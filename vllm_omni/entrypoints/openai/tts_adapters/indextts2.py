...
class IndexTTS25Adapter(IndexTTS2Adapter):
    ...
    def _validate_extra_params(self, extras: Mapping[str, Any]) -> str | None:
        ...
        if "target_duration_ms" in extras:
            value = extras["target_duration_ms"]
            if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)):
                return "extra_params.target_duration_ms must be a finite number"
            if float(value) <= 0:
                return "extra_params.target_duration_ms must be greater than zero"
        if "diffusion_steps" in extras:
            value = extras["diffusion_steps"]
            if isinstance(value, bool) or not isinstance(value, int):
                return "extra_params.diffusion_steps must be an integer"
            if not 1 <= value <= 100:
                return "extra_params.diffusion_steps must be between 1 and 100"
    ...
    async def _build_params(
        ...
        if "target_duration_ms" in extras:
            params["target_duration_ms"] = [float(extras["target_duration_ms"])]
        if "diffusion_steps" in extras:
            params["diffusion_steps"] = [int(extras["diffusion_steps"])]
        return params
