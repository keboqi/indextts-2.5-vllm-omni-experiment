...
class IndexTTS2S2MelDecoder(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        target_duration_ms = self._target_duration_ms(request_infos, len(code_lens_list))
        diffusion_steps = self._request_diffusion_steps(request_infos, len(code_lens_list))
        ...
        target_lengths_list = self._target_lengths(
            ...
            sample_rate=int(self.config.s2mel["preprocess_params"]["sr"]),
            hop_length=int(self.config.s2mel["preprocess_params"]["spect_params"]["hop_length"]),
        )
    ...
    @staticmethod
    def _target_duration_ms(infos: list[dict[str, Any]], batch_size: int) -> list[float | None]:
        values = [info.get("target_duration_ms") for info in infos] if infos else [None] * batch_size
        if len(values) != batch_size:
            raise ValueError(f"IndexTTS target-duration batch mismatch: values={len(values)} batch={batch_size}")
        normalized = [None if value is None else float(value) for value in values]
        if any(value is not None and (not math.isfinite(value) or value <= 0) for value in normalized):
            raise ValueError("IndexTTS target_duration_ms values must be finite and greater than zero")
        return normalized

    def _request_diffusion_steps(self, infos: list[dict[str, Any]], batch_size: int) -> list[int]:
        values = [int(info.get("diffusion_steps", self.diffusion_steps)) for info in infos]
        if not infos:
            values = [self.diffusion_steps] * batch_size
        if len(values) != batch_size:
            raise ValueError(f"IndexTTS diffusion-step batch mismatch: values={len(values)} batch={batch_size}")
        if any(not 1 <= value <= 100 for value in values):
            raise ValueError("IndexTTS diffusion_steps values must be between 1 and 100")
        return values
    ...
    def _target_lengths(
        ...
        *,
        ...
        target_duration_ms: list[float | None] | None = None,
        ...
        targets = target_duration_ms if target_duration_ms is not None else [None] * len(code_lens)
        if len(targets) != len(code_lens):
            raise ValueError(
                f"IndexTTS target-duration batch mismatch: values={len(targets)} codes={len(code_lens)}"
            )
    ...
    def _run_cfm_groups(
        ...
        *,
        ...
        seeds: list[int | None],
        diffusion_steps: list[int] | None = None,
        max_group_size: int,
    ) -> list[torch.Tensor]:
        ...
        request_steps = diffusion_steps if diffusion_steps is not None else [self.diffusion_steps] * batch_size
        if not (len(target_lengths) == len(ref_lengths) == len(seeds) == len(request_steps) == batch_size):
            raise ValueError(
                "IndexTTS CFM input batch mismatch: "
                f"batch={batch_size} target={len(target_lengths)} "
                f"ref={len(ref_lengths)} seeds={len(seeds)} steps={len(request_steps)}"
            )
        if batch_size == 0:
            return []
        ...
        for step_count in dict.fromkeys(request_steps[index] for index in sorted_indices):
            step_indices = [index for index in sorted_indices if request_steps[index] == step_count]
            groups.extend(
                self._cfm_group_indices(
                    sorted_indices=step_indices,
        ...
        for group_indices in groups:
            ...
            group_steps = [request_steps[index] for index in group_indices]
            if len(set(group_steps)) != 1:
                raise RuntimeError("IndexTTS CFM group contains mixed diffusion step counts")
            group_x_lengths = [
