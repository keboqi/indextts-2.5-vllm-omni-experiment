...
class IndexTTS2TalkerForConditionalGeneration(nn.Module):
    ...
    def make_omni_output(self, model_outputs: torch.Tensor | OmniOutput, **kwargs: Any) -> OmniOutput:
        ...
        for info in info_dicts:
            ...
            if mel_len == 1:
                ...
                for key in ("target_duration_ms", "diffusion_steps"):
                    if info_meta.get(key) is not None:
                        req_meta[key] = info_meta[key]
    ...
    def _preprocess_prefill(
        ...
        target_duration_raw = _first("target_duration_ms")
        target_duration_ms = float(target_duration_raw) if target_duration_raw is not None else None
        diffusion_steps_raw = _first("diffusion_steps")
        diffusion_steps = int(diffusion_steps_raw) if diffusion_steps_raw is not None else None
        ...
        if target_duration_ms is not None and (not math.isfinite(target_duration_ms) or target_duration_ms <= 0):
            raise ValueError("IndexTTS 2.5 target_duration_ms must be a finite number greater than zero")
        if diffusion_steps is not None and not 1 <= diffusion_steps <= 100:
            raise ValueError("IndexTTS 2.5 diffusion_steps must be between 1 and 100")
