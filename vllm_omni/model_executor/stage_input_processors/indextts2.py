...
def _build_s2mel_additional_information(
    ...
    *,
    ...
    target_duration_ms = meta.get("target_duration_ms")
    if target_duration_ms is not None:
        additional_information["target_duration_ms"] = float(target_duration_ms)
    diffusion_steps = meta.get("diffusion_steps")
    if diffusion_steps is not None:
        additional_information["diffusion_steps"] = int(diffusion_steps)
...
def talker2s2mel_full_payload(
    ...
    **_: Any,
    ...
    meta = {
        ...
        "target_duration_ms": _get_payload_value(
            pooling_output,
            "meta.target_duration_ms",
            "meta",
            "target_duration_ms",
        ),
        "diffusion_steps": _get_payload_value(
            pooling_output,
            "meta.diffusion_steps",
            "meta",
            "diffusion_steps",
        ),
    }
