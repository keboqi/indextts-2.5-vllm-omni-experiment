...
def _indextts25_adapter_and_request(*, speed: float, extra_params=None):
...
def test_indextts25_passes_exact_target_duration_to_stage_one():
    adapter, request = _indextts25_adapter_and_request(
        speed=1.0,
        extra_params={"target_duration_ms": 1234},
    )

    assert adapter.validate(request) is None
    assert asyncio.run(adapter._build_params(request))["target_duration_ms"] == [1234.0]


def test_indextts25_passes_request_local_diffusion_steps_to_stage_one():
    adapter, request = _indextts25_adapter_and_request(
        speed=1.0,
        extra_params={"diffusion_steps": 12},
    )

    assert adapter.validate(request) is None
    assert asyncio.run(adapter._build_params(request))["diffusion_steps"] == [12]


@pytest.mark.parametrize("value", [True, 0, 101, 1.5, "10"])
def test_indextts25_rejects_invalid_diffusion_steps(value):
    adapter, request = _indextts25_adapter_and_request(
        speed=1.0,
        extra_params={"diffusion_steps": value},
    )

    assert "diffusion_steps" in adapter.validate(request)


@pytest.mark.parametrize("value", [True, 0, -1, float("nan"), "1000"])
def test_indextts25_rejects_invalid_exact_target_duration(value):
    adapter, request = _indextts25_adapter_and_request(
        speed=1.0,
        extra_params={"target_duration_ms": value},
    )

    assert "target_duration_ms" in adapter.validate(request)
