...
def test_prefill_preserves_exact_target_duration_in_metadata(monkeypatch):
    talker = _make_cached_v25_prefill_talker(monkeypatch)
    ...
    assert info_update["meta"]["target_duration_ms"] == 1234.0


def test_prefill_preserves_diffusion_steps_in_metadata(monkeypatch):
    talker = _make_cached_v25_prefill_talker(monkeypatch)
    ...
    assert info_update["meta"]["diffusion_steps"] == 12
...
def test_no_latent_sampled_token_payload_emits_only_first_step_metadata():
    ...
    assert first_decode_output.multimodal_outputs["meta"][0]["target_duration_ms"] == 1234.0
    assert first_decode_output.multimodal_outputs["meta"][0]["diffusion_steps"] == 12
