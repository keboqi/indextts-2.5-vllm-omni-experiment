...
def test_s2mel_exact_duration_overrides_duration_factor():
    assert IndexTTS2S2MelDecoder._target_lengths(
        [100, 80],
        semantic_time_scale=2,
        mel_code_to_frame_ratio=1.72,
...
def test_s2mel_exact_duration_validation():
    ...
    with pytest.raises(ValueError, match="finite and greater than zero"):
        IndexTTS2S2MelDecoder._target_duration_ms(
            [{"target_duration_ms": 0}],
            batch_size=1,
        )


def test_s2mel_request_local_diffusion_steps():
    decoder = object.__new__(IndexTTS2S2MelDecoder)
    decoder.diffusion_steps = 25
    assert decoder._request_diffusion_steps(
        [{"diffusion_steps": 10}, {}],
        batch_size=2,
    ) == [10, 25]
    with pytest.raises(ValueError, match="between 1 and 100"):
        decoder._request_diffusion_steps(
            [{"diffusion_steps": 0}],
            batch_size=1,
        )
